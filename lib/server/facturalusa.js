...
const { resolveStoredCheckoutCustomer } = require('./order-flow');
const FACTURALUSA_CANONICAL_BASE_URL = 'https://facturalusa.pt/api/v2';
const FACTURALUSA_ZERO_VAT_EXEMPTION_FALLBACK = 'M10';
...
function getFacturalusaConfig() {
    ...
    const baseUrl = normalizeFacturalusaBaseUrl(
        mode === 'test'
...
function shouldForceEmitOnPaid() {
    return String(process.env.FACTURALUSA_FORCE_EMIT_ON_PAID || 'true').toLowerCase() !== 'false';
}
...
function appendFormEntry(pairs, key, value) {
    ...
    if (Array.isArray(value)) {
        value.forEach((item, index) => {
            appendFormEntry(pairs, `${key}[${index}]`, item);
        });
        return;
    }
    ...
    if (isPlainObject(value)) {
        Object.entries(value).forEach(([childKey, childValue]) => {
            appendFormEntry(pairs, `${key}[${childKey}]`, childValue);
        });
        return;
    }
...
function toFormUrlEncoded(body) {
    ...
    Object.entries(body).forEach(([key, value]) => {
        appendFormEntry(pairs, key, value);
    });
    return pairs.join('&');
}

function normalizeFacturalusaBaseUrl(raw) {
    let value = normalizeText(raw);
    if (!value) {
        return FACTURALUSA_CANONICAL_BASE_URL;
    }
    ...
    let path = normalizeText(parsed.pathname).replace(/\/+$/, '');
...
function resolveFacturalusaSaleId(sale) {
    const raw = sale?.id ?? sale?.sale_id ?? sale?.saleId ?? sale?.document_id ?? null;
...
function resolveFacturalusaSaleDocumentInfo(sale) {
    ...
    return {
        saleId: resolveFacturalusaSaleId(sale),
        documentNumber: firstNonEmptyValue(
            sale.document_full_number,
            sale.document_number,
            sale.number,
            sale.reference
        ),
        documentUrl: firstNonEmptyValue(
            sale.url_file,
            sale.url,
            sale.file_url,
            sale.download_url
...
function assertFacturalusaSaleResponse(sale) {
    if (!sale || typeof sale !== 'object' || Array.isArray(sale)) {
        const error = new Error('Facturalusa devolveu uma resposta de venda inválida.');
        error.code = 'FACTURALUSA_INVALID_SALE_RESPONSE';
...
function resolveFacturalusaVatExemptionCodeFromSnapshot(fiscalSnapshot = {}, context = {}) {
    const vatRate = Number(context?.vatRate ?? resolveFacturalusaVatRateFromSnapshot(fiscalSnapshot));
    const rawCode = String(fiscalSnapshot?.vat_regime_code || '').trim().toUpperCase();

    if (vatRate === 0) {
        return rawCode || FACTURALUSA_ZERO_VAT_EXEMPTION_FALLBACK;
    }
...
function classifyFacturalusaError(error) {
    ...
    if (error?.code === 'FACTURALUSA_UNEXPECTED_HTML_RESPONSE') {
        return {
            code: 'FACTURALUSA_UNEXPECTED_HTML_RESPONSE',
            message: error?.message || 'A Facturalusa devolveu HTML inesperado em vez de JSON de API. Verifique FACTURALUSA_BASE_URL e token do ambiente ativo.',
            retryable: false
        };
    }

    if (error?.code === 'FACTURALUSA_DOCUMENT_URL_MISSING') {
        return {
            code: 'FACTURALUSA_DOCUMENT_URL_MISSING',
            message: error?.message || 'A Facturalusa não devolveu URL do PDF após emissão.',
            retryable: true
        };
    }
    ...
    if (error?.code === 'FACTURALUSA_INVALID_TAX_ID') {
        return {
            ...
            message: error?.message || 'A Facturalusa rejeitou o NIF fornecido pelo cliente.',
            retryable: false
        };
    }
    ...
    if (text.includes('nif') && (text.includes('invalid') || text.includes('invalido') || text.includes('inv\u00e1lido'))) {
        return {
            code: 'FACTURALUSA_INVALID_TAX_ID',
            message: 'A Facturalusa rejeitou o NIF fornecido pelo cliente.',
            retryable: false
        };
    }
...
async function facturalusaRequest(path, options = {}) {
    ...
    const baseUrl = normalizeFacturalusaBaseUrl(options.baseUrlOverride || config.baseUrl);
    const endpoint = `${baseUrl}${path.startsWith('/') ? path : `/${path}`}`;
    const method = String(options.method || 'POST').toUpperCase();
    const headers = {
        Authorization: `Bearer ${token}`,
        Accept: 'application/json',
        'User-Agent': 'IberFlag/1.0 (+https://iberflag.com)',
    ...
    if (body && !(body instanceof FormData)) {
        if (options.form === true) {
            ...
            body = typeof body === 'string' ? body : toFormUrlEncoded(body);
        } else {
    ...
    if (options.expectJson !== false) {
        if (payload === null || payload === undefined || payload === '') {
            const error = new Error(`Facturalusa devolveu resposta vazia em ${path}.`);
            error.code = 'FACTURALUSA_EMPTY_RESPONSE';
            error.retryable = true;
            throw error;
        }

        if (typeof payload === 'string') {
            if (isLikelyHtmlPayload(payload) || responseContentType.includes('text/html')) {
                if (!options._retriedCanonicalBaseUrl && baseUrl !== FACTURALUSA_CANONICAL_BASE_URL) {
                    return facturalusaRequest(path, {
                        ...options,
                        baseUrlOverride: FACTURALUSA_CANONICAL_BASE_URL,
                        _retriedCanonicalBaseUrl: true
                    });
                }
                const error = new Error('Facturalusa devolveu HTML inesperado (possível sessão/login em vez de API JSON).');
                error.code = 'FACTURALUSA_UNEXPECTED_HTML_RESPONSE';
                error.retryable = false;
                error.response = payload.slice(0, 400);
                throw error;
            }

            const error = new Error(`Facturalusa devolveu texto inesperado em ${path}.`);
            error.code = 'FACTURALUSA_UNEXPECTED_TEXT_RESPONSE';
            error.retryable = true;
            error.response = payload.slice(0, 400);
            throw error;
        }
...
async function resolveFacturalusaSaleDownloadUrl(sale) {
    const info = resolveFacturalusaSaleDocumentInfo(sale);
    if (info.documentUrl) {
    ...
    const payload = await facturalusaRequest(`/sales/${encodeURIComponent(String(info.saleId))}/download`, {
        body: {
            format: 'A4',
            language: 'PT'
        }
    ...
    return firstNonEmptyValue(payload.url, payload.url_file, payload.file_url, payload.download_url);
}

async function ensureFacturalusaSaleDocumentUrl(sale) {
    assertFacturalusaSaleResponse(sale);
    const info = resolveFacturalusaSaleDocumentInfo(sale);
    if (info.documentUrl) {
        return sale;
    }

    const resolvedDownloadUrl = await resolveFacturalusaSaleDownloadUrl(sale);
    ...
    return {
        ...sale,
        url_file: sale.url_file || resolvedDownloadUrl,
        url: sale.url || resolvedDownloadUrl
    };
}
...
async function createCustomer(customer) {
    ...
    const payload = {
        ...
        country: resolveFacturalusaCountry(customer) || 'Portugal',
    ...
    const address = String(customer?.morada || customer?.address || '').trim();
    const city = String(customer?.cidade || customer?.city || '').trim();
    const postalCode = String(customer?.codigo_postal || customer?.postal_code || '').trim();
    const phone = String(customer?.telefone || customer?.phone || '').trim();
...
async function findOrCreateCustomer(customer, context = {}) {
    ...
    try {
        return await createCustomer(customer, context);
    } catch (createError) {
        if (!email) {
            throw createError;
        }
...
async function createItem(item, context = {}) {
    ...
    const vatRate = resolveFacturalusaVatRateFromSnapshot(fiscalSnapshot);
    const payload = {
        ...
        unit: 'uni',
        vat: vatRate,
        vat_exemption: resolveFacturalusaVatExemptionCodeFromSnapshot(fiscalSnapshot, { vatRate }),
        type: 'Produtos acabados e intermédios',
        cost_price: 0,
        observations: String(context.observations || buildFacturalusaItemObservations(item)).trim() || undefined
    };
...
async function findOrCreateItem(item, context = {}) {
    ...
    try {
        return await createItem(item, context);
    } catch (createError) {
...
async function createSaleForOrder(orderContext) {
    ...
    const vatRate = resolveFacturalusaVatRateFromSnapshot(fiscalSnapshot);
    const vatExemptionCode = resolveFacturalusaVatExemptionCodeFromSnapshot(fiscalSnapshot, { vatRate });
    let seriesId = resolveFacturalusaSeriesId();
    const items = Array.isArray(cartItems) ? cartItems : [];
    const customerAddress = String(sourceCustomer?.morada || sourceCustomer?.address || customer?.address || '').trim();
    const customerCity = String(sourceCustomer?.cidade || sourceCustomer?.city || customer?.city || '').trim();
    const customerPostalCode = String(sourceCustomer?.codigo_postal || sourceCustomer?.postal_code || customer?.postal_code || '').trim();
    const customerCountry = resolveFacturalusaCountry({
        ...(sourceCustomer || {}),
        country: sourceCustomer?.country || customer?.country || ''
    });
    const customerCurrencyId = Number(customer?.currency_id);
    const customerPriceId = Number(customer?.price_id);

    const savedItems = [];
    for (const item of items) {
        const product = await findOrCreateItem(item, {
            observations: buildFacturalusaItemObservations(item),
            fiscalSnapshot
        });
        savedItems.push({
            ...
            vat: vatRate,
            vat_exemption: vatExemptionCode
        });
    }

    const saleDraftPayload = {
        issue_date: new Date().toISOString().slice(0, 10),
        document_type: documentType,
        ...(seriesId ? { serie_id: Number(seriesId) || seriesId } : {}),
        ...
        ...(currency ? { currency } : {}),
        ...(Number.isFinite(customerCurrencyId) && customerCurrencyId > 0 ? { currency_id: customerCurrencyId } : {}),
        ...(Number.isFinite(customerPriceId) && customerPriceId > 0 ? { price_id: customerPriceId } : {}),
        vat_type: resolveFacturalusaVatTypeFromSnapshot(fiscalSnapshot),
        status: 'Rascunho',
        items: savedItems,
        language: resolveFacturalusaLanguage(),
        observations: buildFacturalusaOrderObservations(order)
    };
    ...
    let saleDraft;
    try {
        saleDraft = await facturalusaRequest('/sales', {
            body: saleDraftPayload
        });
    } catch (saleDraftError) {
        const classified = classifyFacturalusaError(saleDraftError);
        if (seriesId && classified.code === 'FACTURALUSA_SERIE_NOT_COMMUNICATED') {
            console.warn('Serie Facturalusa configurada nao comunicada, a repetir criacao sem serie explicita.', {
                configuredSeriesId: seriesId,
                documentType
            });
            saleDraft = await facturalusaRequest('/sales', {
                body: {
            ...
            throw saleDraftError;
        }
    }

    assertFacturalusaSaleResponse(saleDraft);

    const saleId = resolveFacturalusaSaleId(saleDraft);
    if (!saleId) {
        const error = new Error('Facturalusa nao devolveu o identificador da venda em rascunho.');
        error.code = 'FACTURALUSA_INVALID_SALE_RESPONSE';
        error.retryable = true;
        error.response = saleDraft;
        throw error;
    }

    const saleFinalizePayload = {
        ...saleDraftPayload,
        serie_id: saleDraft.serie_id || saleDraftPayload.serie_id,
        currency_id: saleDraft.currency_id || saleDraftPayload.currency_id,
        price_id: saleDraft.price_id || saleDraftPayload.price_id,
        status: 'Terminado'
    };

    const sale = await facturalusaRequest(`/sales/${encodeURIComponent(String(saleId))}`, {
        method: 'PUT',
        body: saleFinalizePayload
    });

    assertFacturalusaSaleResponse(sale);
    const saleWithDocument = await ensureFacturalusaSaleDocumentUrl(sale);
    const saleInfo = resolveFacturalusaSaleDocumentInfo(saleWithDocument);
    if (!saleInfo.documentUrl) {
        const error = new Error('Facturalusa nao devolveu URL de download do documento emitido.');
        error.code = 'FACTURALUSA_DOCUMENT_URL_MISSING';
        error.retryable = true;
        error.response = saleWithDocument;
        throw error;
    }
...
async function issueFacturalusaDocumentForOrder(orderContext) {
    ...
    if (!shouldForceEmitOnPaid() && (fiscalSnapshot.fiscal_decision_mode !== 'auto_emit' || fiscalSnapshot.invoice_state !== 'ready_to_emit')) {
    ...
    const storedCustomer = resolveStoredCheckoutCustomer(order, split.meta || {}, providedCustomer || {});
    const customerData = {
        ...storedCustomer,
        email: storedCustomer.email || session?.customer_email || providedCustomer?.email || ''
    };
...
module.exports = {
    ...
    ensureFacturalusaSaleDocumentUrl
};
