...
async function createSaleForOrder(orderContext) {
    ...
    if (seriesId) {
        try {
            ...
            const classified = classifyFacturalusaError(seriesError);
            if (classified.code === 'FACTURALUSA_SERIE_NOT_COMMUNICATED') {
                // Fallback: let Facturalusa pick the account default series for this document type.
                console.warn('Serie Facturalusa configurada nao comunicada para este documento, a tentar com serie pre-definida:', {
    ...
    const sale = await facturalusaRequest('/sales', {
        body: {
    ...
    assertFacturalusaSaleResponse(sale);
    const enrichedSale = await ensureFacturalusaSaleDocumentUrl(sale);
    const saleInfo = resolveFacturalusaSaleDocumentInfo(enrichedSale);
    if (!saleInfo.documentUrl) {
        const error = new Error('Facturalusa não devolveu URL de download do documento emitido.');
        error.code = 'FACTURALUSA_DOCUMENT_URL_MISSING';
        error.retryable = true;
        error.response = enrichedSale;
        throw error;
    }
