...
async function createSaleForOrder(orderContext) {
    ...
    let sale;
    try {
        sale = await facturalusaRequest('/sales', {
            body: salePayload,
            form: true
        });
    } catch (saleError) {
        const classified = classifyFacturalusaError(saleError);
        if (seriesId && classified.code === 'FACTURALUSA_SERIE_NOT_COMMUNICATED') {
            console.warn('Serie Facturalusa configurada nao comunicada, a repetir emissao sem serie explicita.', {
