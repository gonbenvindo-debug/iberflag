...
const PORTUGAL_PLACE_MUNICIPALITY_OVERRIDES = new Map([
...
function resolveMunicipalityFromPostalPlace(placeName = '', country = 'PT') {
    ...
    return PORTUGAL_PLACE_MUNICIPALITY_OVERRIDES.get(normalizeLookupKey(placeName)) || '';
}
...
async function resolvePostalLookup({
    ...
    const city = pickString(place, ['place name']);
    let municipality = normalizedCountry === 'PT' ? '' : city;
    ...
    if (!municipality) {
        municipality = resolveMunicipalityFromPostalPlace(city, normalizedCountry);
    }
