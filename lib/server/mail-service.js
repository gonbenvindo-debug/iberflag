...
async function sendViaResend({ to, subject, html, text, preheader = '', replyTo = '' }) {
...
async function sendViaSmtp({ to, subject, html, text, preheader = '', replyTo = '' }) {
    const config = getSmtpConfig();
...
async function sendTemplateEmail({ to, subject, html, text, preheader = '', replyTo = '' }) {
    const recipients = Array.isArray(to)
        ? [...new Set(to.map((value) => String(value || '').trim()).filter(Boolean))]
        : [String(to || '').trim()].filter(Boolean);
    ...
    if (provider === 'resend') {
        return sendViaResend({ to: recipients, subject, html, text, preheader, replyTo });
    }

    if (provider === 'smtp') {
        return sendViaSmtp({ to: recipients, subject, html, text, preheader, replyTo });
    }
