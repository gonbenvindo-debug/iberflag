const { splitOrderNotesAndMeta } = require('./checkout');
const {
    buildOrderEmailContext,
    renderEmailTemplate,
    sanitizeTemplateRow
} = require('./email-templates');
const { sendTemplateEmail } = require('./mail-service');
const { resolveActiveMailProvider, getMailIdentity } = require('./mail-config');
const { getPaymentEnvironment } = require('./stripe');
const { getEnvValue } = require('./env');
const {
    ensureFacturalusaSaleDocumentUrl,
    resolveFacturalusaSaleDocumentInfo,
    facturalusaRequest
} = require('./facturalusa');
const { queueReviewItem, resolveReviewItem, logOperationalEvent } = require('./ops');

function normalizeText(value) {
    return String(value ?? '').trim();
}

async function optionalLogDelivery(supabase, payload) {
    try {
        await supabase
            .from('email_delivery_logs')
            .insert(payload);
    } catch (error) {
        console.warn('Nao foi possivel registar email_delivery_logs:', error?.message || error);
    }
}

async function findTemplate(supabase, templateKey) {
    const { data, error } = await supabase
        .from('email_templates')
        .select('*')
        .eq('template_key', templateKey)
        .eq('active', true)
        .maybeSingle();

    if (error) throw error;
    return sanitizeTemplateRow(data);
}

async function optionalLogOperational(supabase, payload) {
    try {
        await logOperationalEvent(supabase, payload);
    } catch (error) {
        console.warn('Nao foi possivel registar operational_logs para email:', error?.message || error);
    }
}

const TEMPLATE_FALLBACK_KEYS = {
    order_confirmation_preparacao: ['order_confirmation'],
    invoice_issued_with_attachment: ['invoice_document_ready'],
    order_status_expedido: ['order_status_update'],
    order_status_entregue: ['order_status_update']
};

async function resolveTemplateWithFallback(supabase, templateKey) {
    const orderedKeys = [templateKey, ...(TEMPLATE_FALLBACK_KEYS[templateKey] || [])]
        .map((value) => String(value || '').trim())
        .filter(Boolean);

    for (const key of orderedKeys) {
        const template = await findTemplate(supabase, key);
        if (template) {
            return {
                template,
                resolvedTemplateKey: key
            };
        }
    }

    return {
        template: null,
        resolvedTemplateKey: ''
    };
}

async function hasSentDedupe(supabase, dedupeKey) {
    if (!dedupeKey) return false;

    try {
        const { data, error } = await supabase
            .from('email_delivery_logs')
            .select('id')
            .eq('dedupe_key', dedupeKey)
            .eq('status', 'sent')
            .maybeSingle();

        if (error) throw error;
        return Boolean(data?.id);
    } catch (error) {
        const raw = String(error?.message || '').toLowerCase();
        if (error?.code === 'PGRST205' || raw.includes('email_delivery_logs') || raw.includes('dedupe_key')) {
            return false;
        }
        throw error;
    }
}

async function findOrderCustomer(supabase, order, meta) {
    if (order?.cliente_id) {
        try {
            const { data, error } = await supabase
                .from('clientes')
                .select('*')
                .eq('id', order.cliente_id)
                .maybeSingle();

            if (error) throw error;
            if (data) return data;
        } catch (error) {
            console.warn('Nao foi possivel carregar cliente para email:', error?.message || error);
        }
    }

    const checkoutCustomer = meta?.checkoutCustomer || order?.checkout_payload?.customer || order?.checkout_payload?.checkoutCustomer || {};
    return {
        nome: checkoutCustomer.nome || checkoutCustomer.name || 'Cliente',
        email: checkoutCustomer.email || '',
        telefone: checkoutCustomer.telefone || checkoutCustomer.phone || '',
        empresa: checkoutCustomer.empresa || checkoutCustomer.company || '',
        nif: checkoutCustomer.nif || checkoutCustomer.tax_id || '',
        morada: checkoutCustomer.morada || checkoutCustomer.address || '',
        codigo_postal: checkoutCustomer.codigo_postal || checkoutCustomer.postal_code || '',
        cidade: checkoutCustomer.cidade || checkoutCustomer.city || '',
        country: checkoutCustomer.country || checkoutCustomer.countryCode || checkoutCustomer.country_code || 'PT',
        tipo_cliente: checkoutCustomer.tipo_cliente || checkoutCustomer.customerType || 'particular'
    };
}

function resolveInvoiceDocument(order, meta) {
    return {
        documentNumber: normalizeText(order?.facturalusa_document_number) || normalizeText(meta?.facturalusaDocumentNumber),
        documentUrl: normalizeText(order?.facturalusa_document_url) || normalizeText(meta?.facturalusaDocumentUrl)
    };
}

function resolveFacturalusaPayloadObject(order) {
    const raw = order?.facturalusa_payload;
    if (!raw) return null;
    if (typeof raw === 'object' && !Array.isArray(raw)) return raw;
    if (typeof raw !== 'string') return null;
    const trimmed = raw.trim();
    if (!trimmed.startsWith('{')) return null;
    try {
        const parsed = JSON.parse(trimmed);
        return (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) ? parsed : null;
    } catch {
        return null;
    }
}

async function resolveInvoiceDocumentFromFacturalusa(order, supabase) {
    const salePayload = resolveFacturalusaPayloadObject(order);
    if (!salePayload) {
        return { documentNumber: '', documentUrl: '' };
    }

    const enrichedSale = await ensureFacturalusaSaleDocumentUrl(salePayload);
    const info = resolveFacturalusaSaleDocumentInfo(enrichedSale);
    const documentNumber = normalizeText(info.documentNumber);
    const documentUrl = normalizeText(info.documentUrl);

    if (documentUrl && order?.id && supabase) {
        try {
            await supabase
                .from('encomendas')
                .update({
                    facturalusa_document_number: documentNumber || null,
                    facturalusa_document_url: documentUrl || null,
                    facturalusa_payload: enrichedSale || salePayload
                })
                .eq('id', order.id);
        } catch (error) {
            console.warn('Nao foi possivel guardar URL da fatura resolvida por download:', error?.message || error);
        }
    }

    return { documentNumber, documentUrl };
}

function buildInvoiceAttachmentFilename(orderCode, documentNumber) {
    const baseCode = normalizeText(documentNumber) || normalizeText(orderCode) || 'documento';
    const safeToken = baseCode.replace(/[^a-zA-Z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || 'documento';
    return `Fatura-${safeToken}.pdf`;
}

function isPdfBuffer(buffer) {
    if (!Buffer.isBuffer(buffer) || buffer.length < 4) return false;
    return buffer.subarray(0, 4).toString('utf8') === '%PDF';
}

function resolveFacturalusaApiToken() {
    const mode = getPaymentEnvironment();
    if (mode === 'test') {
        return normalizeText(getEnvValue(['FACTURALUSA_API_TOKEN_TEST', 'FACTURALUSA_BEARER_TOKEN_TEST']));
    }
    return normalizeText(getEnvValue([
        'FACTURALUSA_API_TOKEN_LIVE',
        'FACTURALUSA_BEARER_TOKEN_LIVE',
        'FACTURALUSA_API_TOKEN',
        'FACTURALUSA_BEARER_TOKEN'
    ]));
}

function isFacturalusaHost(url) {
    try {
        const parsed = new URL(String(url || '').trim());
        return String(parsed.hostname || '').toLowerCase().endsWith('facturalusa.pt');
    } catch {
        return false;
    }
}

async function fetchPdfDocument(url) {
    const headers = {
        Accept: 'application/pdf'
    };
    const facturalusaToken = resolveFacturalusaApiToken();
    if (facturalusaToken && isFacturalusaHost(url)) {
        headers.Authorization = `Bearer ${facturalusaToken}`;
    }

    let response;
    try {
        response = await fetch(url, {
            method: 'GET',
            headers
        });
    } catch (error) {
        const nextError = new Error('Falha ao descarregar PDF da fatura.');
        nextError.code = 'INVOICE_ATTACHMENT_FETCH_FAILED';
        nextError.cause = error;
        throw nextError;
    }

    if (!response.ok) {
        const error = new Error(`Falha ao descarregar PDF da fatura (HTTP ${response.status}).`);
        error.code = 'INVOICE_ATTACHMENT_FETCH_BAD_STATUS';
        throw error;
    }

    const contentType = normalizeText(response.headers.get('content-type')).toLowerCase();
    const content = Buffer.from(await response.arrayBuffer());
    if (!content.length) {
        const error = new Error('PDF da fatura vazio.');
        error.code = 'INVOICE_ATTACHMENT_EMPTY';
        throw error;
    }

    if (!contentType.includes('pdf') && !isPdfBuffer(content)) {
        const error = new Error('URL da fatura nao devolveu um PDF valido.');
        error.code = 'INVOICE_ATTACHMENT_INVALID_CONTENT';
        throw error;
    }

    return content;
}

async function resolveInvoiceDownloadUrlFromSale(order) {
    const salePayload = resolveFacturalusaPayloadObject(order);
    if (!salePayload) return '';
    const saleInfo = resolveFacturalusaSaleDocumentInfo(salePayload);
    if (!saleInfo.saleId) return '';

    const payload = await facturalusaRequest(`/sales/${encodeURIComponent(String(saleInfo.saleId))}/download`, {
        body: {
            format: 'A4',
            language: 'PT'
        }
    });

    if (typeof payload === 'string') {
        const text = normalizeText(payload);
        return /^https?:\/\//i.test(text) ? text : '';
    }

    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
        return '';
    }

    return normalizeText(payload.url || payload.url_file || payload.file_url || payload.download_url);
}

async function fetchInvoiceAttachmentOrThrow(order, meta, supabase = null) {
    const invoice = resolveInvoiceDocument(order, meta);
    let documentNumber = normalizeText(invoice.documentNumber);
    let documentUrl = normalizeText(invoice.documentUrl);

    if (!documentUrl) {
        const resolved = await resolveInvoiceDocumentFromFacturalusa(order, supabase);
        documentNumber = documentNumber || normalizeText(resolved.documentNumber);
        documentUrl = documentUrl || normalizeText(resolved.documentUrl);
    }

    if (!documentUrl) {
        const error = new Error('Documento fiscal sem URL de PDF.');
        error.code = 'INVOICE_ATTACHMENT_URL_MISSING';
        throw error;
    }

    let content = null;
    try {
        content = await fetchPdfDocument(documentUrl);
    } catch (primaryError) {
        const fallbackUrl = await resolveInvoiceDownloadUrlFromSale(order).catch(() => '');
        if (!fallbackUrl || fallbackUrl === documentUrl) {
            throw primaryError;
        }
        documentUrl = fallbackUrl;
        content = await fetchPdfDocument(documentUrl);

        if (order?.id && supabase) {
            try {
                await supabase
                    .from('encomendas')
                    .update({
                        facturalusa_document_url: documentUrl
                    })
                    .eq('id', order.id);
            } catch (updateError) {
                console.warn('Nao foi possivel atualizar URL fallback da fatura:', updateError?.message || updateError);
            }
        }
    }

    return [{
        filename: buildInvoiceAttachmentFilename(order?.numero_encomenda, documentNumber),
        content,
        contentType: 'application/pdf'
    }];
}

function buildNotificationOrder(order, meta, statusOverride = '') {
    const workflowStatus = statusOverride || meta?.workflowStatus || order?.status || 'em_preparacao';
    return {
        ...order,
        status: workflowStatus,
        tracking_codigo: meta?.trackingCode || order?.tracking_codigo || order?.codigo_tracking || order?.numero_encomenda || '',
        tracking_url: meta?.trackingUrl || order?.tracking_url || '',
        email_meta: meta || {}
    };
}

function isInvoiceIssuedTemplate(templateKey) {
    return String(templateKey || '').trim() === 'invoice_issued_with_attachment';
}

function isEmailProviderMissingInLiveEnvironment() {
    return getPaymentEnvironment() === 'live' && resolveActiveMailProvider() === 'none';
}

async function queueInvoiceEmailRetry(supabase, order, details, extraPayload = {}) {
    if (!supabase || !order?.id) {
        return;
    }

    const mailIdentity = getMailIdentity();
    await queueReviewItem(supabase, {
        queue_key: `invoice-email:${order.id}`,
        order_id: order.id,
        type: 'invoice_email_retry',
        priority: 'high',
        title: 'Email da fatura pendente/falhou',
        details: details || 'Falha ao enviar email da fatura.',
        payload: {
            orderCode: order.numero_encomenda || '',
            fromEmail: mailIdentity.fromEmail || '',
            provider: resolveActiveMailProvider(),
            ...extraPayload
        }
    }).catch(() => {});
}

async function sendOrderEmailNotification({
    supabase,
    req = null,
    order,
    templateKey,
    statusOverride = '',
    dedupeKey = '',
    requireInvoiceAttachment = false,
    attachments = [],
    throwOnError = false
}) {
    if (!order?.id || !templateKey) {
        return { sent: false, skipped: true, reason: 'MISSING_ORDER_OR_TEMPLATE' };
    }

    try {
        if (isInvoiceIssuedTemplate(templateKey) && isEmailProviderMissingInLiveEnvironment()) {
            const reasonMessage = 'Provider de email proprio nao configurado em producao.';
            const split = splitOrderNotesAndMeta(order.notas || '');
            const recipientGuess = String(
                split.meta?.checkoutCustomer?.email
                || order?.checkout_payload?.customer?.email
                || order?.checkout_payload?.checkoutCustomer?.email
                || ''
            ).trim() || null;
            await optionalLogDelivery(supabase, {
                template_key: templateKey,
                order_id: order.id,
                recipient: recipientGuess,
                subject: null,
                dedupe_key: dedupeKey || null,
                status: 'failed',
                error_message: reasonMessage,
                payload: {
                    type: templateKey,
                    code: 'MAIL_PROVIDER_REQUIRED_IN_LIVE'
                }
            });
            await queueInvoiceEmailRetry(supabase, order, reasonMessage, {
                code: 'MAIL_PROVIDER_REQUIRED_IN_LIVE'
            });
            await optionalLogOperational(supabase, {
                event_name: 'invoice_email_failed',
                level: 'error',
                order_id: order.id,
                payload: {
                    orderCode: order.numero_encomenda || '',
                    reason: reasonMessage,
                    code: 'MAIL_PROVIDER_REQUIRED_IN_LIVE'
                }
            });

            return {
                sent: false,
                skipped: true,
                reason: 'MAIL_PROVIDER_REQUIRED_IN_LIVE',
                message: reasonMessage
            };
        }

        const alreadySent = await hasSentDedupe(supabase, dedupeKey);
        if (alreadySent) {
            if (isInvoiceIssuedTemplate(templateKey)) {
                await resolveReviewItem(supabase, `invoice-email:${order.id}`).catch(() => {});
            }
            return { sent: false, skipped: true, reason: 'DUPLICATE_EMAIL' };
        }

        const { template, resolvedTemplateKey } = await resolveTemplateWithFallback(supabase, templateKey);
        if (!template) {
            return { sent: false, skipped: true, reason: 'EMAIL_TEMPLATE_INACTIVE_OR_MISSING' };
        }

        const split = splitOrderNotesAndMeta(order.notas || '');
        const customer = await findOrderCustomer(supabase, order, split.meta);
        const recipient = String(customer?.email || '').trim();
        if (!recipient) {
            return { sent: false, skipped: true, reason: 'ORDER_EMAIL_MISSING' };
        }

        const context = buildOrderEmailContext({
            req,
            order: buildNotificationOrder(order, split.meta, statusOverride),
            customer
        });
        const rendered = renderEmailTemplate(template, context);
        let finalAttachments = Array.isArray(attachments) ? attachments.filter(Boolean) : [];
        if (requireInvoiceAttachment) {
            await optionalLogOperational(supabase, {
                event_name: 'invoice_attachment_fetch_started',
                level: 'info',
                order_id: order.id,
                payload: {
                    orderCode: order.numero_encomenda || ''
                }
            });
            try {
                finalAttachments = await fetchInvoiceAttachmentOrThrow(order, split.meta, supabase);
                await optionalLogOperational(supabase, {
                    event_name: 'invoice_attachment_fetch_succeeded',
                    level: 'info',
                    order_id: order.id,
                    payload: {
                        orderCode: order.numero_encomenda || '',
                        attachments: finalAttachments.length
                    }
                });
            } catch (attachmentError) {
                await optionalLogDelivery(supabase, {
                    template_key: template.template_key,
                    order_id: order.id,
                    recipient,
                    subject: rendered.subject,
                    dedupe_key: dedupeKey || null,
                    status: 'failed',
                    error_message: attachmentError?.message || 'Falha ao preparar anexo da fatura.',
                    payload: {
                        type: templateKey,
                        resolvedTemplateKey: resolvedTemplateKey || template.template_key,
                        status: context.order.status,
                        code: attachmentError?.code || 'INVOICE_ATTACHMENT_FAILED'
                    }
                });
                await queueInvoiceEmailRetry(
                    supabase,
                    order,
                    attachmentError?.message || 'Falha ao preparar anexo da fatura.',
                    { code: attachmentError?.code || 'INVOICE_ATTACHMENT_FAILED' }
                );
                await optionalLogOperational(supabase, {
                    event_name: 'invoice_attachment_fetch_failed',
                    level: 'warning',
                    order_id: order.id,
                    payload: {
                        orderCode: order.numero_encomenda || '',
                        reason: attachmentError?.message || '',
                        code: attachmentError?.code || 'INVOICE_ATTACHMENT_FAILED'
                    }
                });
                return {
                    sent: false,
                    skipped: true,
                    reason: attachmentError?.code || 'INVOICE_ATTACHMENT_FAILED',
                    message: attachmentError?.message || 'Falha ao preparar anexo da fatura.'
                };
            }
        }

        try {
            const result = await sendTemplateEmail({
                to: recipient,
                subject: rendered.subject,
                preheader: rendered.preheader,
                html: rendered.html,
                text: rendered.text,
                attachments: finalAttachments
            });

            await optionalLogDelivery(supabase, {
                template_key: template.template_key,
                order_id: order.id,
                recipient,
                subject: rendered.subject,
                dedupe_key: dedupeKey || null,
                status: 'sent',
                provider_message_id: result.messageId || null,
                payload: {
                    type: templateKey,
                    resolvedTemplateKey: resolvedTemplateKey || template.template_key,
                    status: context.order.status,
                    sentCopy: result.sentCopy || null
                },
                sent_at: new Date().toISOString()
            });

            if (isInvoiceIssuedTemplate(templateKey)) {
                await resolveReviewItem(supabase, `invoice-email:${order.id}`).catch(() => {});
                await optionalLogOperational(supabase, {
                    event_name: 'invoice_email_sent',
                    level: 'info',
                    order_id: order.id,
                    payload: {
                        orderCode: order.numero_encomenda || '',
                        provider: result.provider || resolveActiveMailProvider(),
                        recipient
                    }
                });
            }

            return {
                sent: true,
                recipient,
                messageId: result.messageId || null,
                sentCopy: result.sentCopy || null
            };
        } catch (sendError) {
            await optionalLogDelivery(supabase, {
                template_key: template.template_key,
                order_id: order.id,
                recipient,
                subject: rendered.subject,
                dedupe_key: dedupeKey || null,
                status: 'failed',
                error_message: sendError?.message || 'Falha ao enviar email.',
                payload: {
                    type: templateKey,
                    resolvedTemplateKey: resolvedTemplateKey || template.template_key,
                    status: context.order.status,
                    code: sendError?.code || 'EMAIL_SEND_FAILED'
                }
            });

            if (isInvoiceIssuedTemplate(templateKey)) {
                await queueInvoiceEmailRetry(
                    supabase,
                    order,
                    sendError?.message || 'Falha ao enviar email da fatura.',
                    { code: sendError?.code || 'EMAIL_SEND_FAILED' }
                );
                await optionalLogOperational(supabase, {
                    event_name: 'invoice_email_failed',
                    level: 'warning',
                    order_id: order.id,
                    payload: {
                        orderCode: order.numero_encomenda || '',
                        reason: sendError?.message || '',
                        code: sendError?.code || 'EMAIL_SEND_FAILED'
                    }
                });
            }

            if (throwOnError) throw sendError;
            return {
                sent: false,
                skipped: true,
                reason: sendError?.code || 'EMAIL_SEND_FAILED',
                message: sendError?.message || 'Falha ao enviar email.'
            };
        }
    } catch (error) {
        if (throwOnError) throw error;
        console.warn('Email de encomenda nao enviado:', error?.message || error);
        return {
            sent: false,
            skipped: true,
            reason: error?.code || 'EMAIL_NOTIFICATION_FAILED',
            message: error?.message || 'Falha ao enviar notificacao por email.'
        };
    }
}

module.exports = {
    sendOrderEmailNotification
};
