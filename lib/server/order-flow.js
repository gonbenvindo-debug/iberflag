const {
    buildCheckoutPayloadSnapshot,
    buildServiceOptionItems,
    buildOrderItemSnapshots,
    normalizeCheckoutServiceOptions,
    normalizeCustomerType,
    normalizePaymentMethodType,
    normalizeWorkflowStatusValue
} = require('./checkout');
const {
    normalizeCountryCode,
    normalizeFiscalSnapshot,
    resolveCheckoutCountryCode,
    resolveStoredFiscalSnapshot
} = require('./fiscal-engine');
const { resolveFacturalusaDocumentState } = require('./invoice-state');
const { normalizeTaxId, validateTaxId } = require('./tax-id');
const { buildDesignAssetUrl } = require('./design-storage');

function normalizeText(value) {
    return String(value ?? '').trim();
}

const PUBLIC_WORKFLOW_STATUS_RANK = {
    em_preparacao: 0,
    em_producao: 1,
    expedido: 2,
    entregue: 3
};

const PUBLIC_LEGACY_TO_WORKFLOW_STATUS = {
    pendente: 'em_preparacao',
    processando: 'em_producao',
    concluido: 'entregue',
    cancelado: 'em_preparacao'
};

const COMMON_EMAIL_DOMAIN_FIXES = {
    'gmail.com.pt': 'gmail.com',
    'gmail.pt': 'gmail.com',
    'gmai.com': 'gmail.com',
    'gmial.com': 'gmail.com',
    'gmail.con': 'gmail.com',
    'hotmai.com': 'hotmail.com',
    'hotmail.con': 'hotmail.com',
    'outlook.con': 'outlook.com'
};

function normalizeName(value) {
    return normalizeText(value).toLowerCase().replace(/\s+/g, ' ');
}

function normalizeQuantity(value) {
    const maxQuantity = Math.max(1, Number.parseInt(process.env.CHECKOUT_MAX_ITEM_QUANTITY || '99', 10) || 99);
    const parsed = Number.parseInt(String(value || '1'), 10);
    const quantity = Number.isFinite(parsed) ? parsed : 1;
    return Math.max(1, Math.min(maxQuantity, quantity));
}

function toNumber(value, fallback = 0) {
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : fallback;
}

function resolveWorkflowStatusFromPublicOrderRow(order = {}) {
    const explicit = normalizeText(
        order?.workflow_status
        || order?.workflowStatus
        || order?.estado_workflow
    );
    if (explicit) {
        return normalizeWorkflowStatusValue(explicit);
    }

    const rawStatus = normalizeText(order?.status || order?.estado);
    if (!rawStatus) {
        return '';
    }

    const normalized = normalizeWorkflowStatusValue(rawStatus);
    if (Object.prototype.hasOwnProperty.call(PUBLIC_WORKFLOW_STATUS_RANK, normalized)) {
        return normalized;
    }

    return PUBLIC_LEGACY_TO_WORKFLOW_STATUS[rawStatus] || 'em_preparacao';
}

function pickMostAdvancedPublicWorkflowStatus(...values) {
    const normalizedValues = values
        .map((value) => normalizeWorkflowStatusValue(value))
        .filter((value) => Object.prototype.hasOwnProperty.call(PUBLIC_WORKFLOW_STATUS_RANK, value));

    if (normalizedValues.length === 0) {
        return 'em_preparacao';
    }

    return normalizedValues.reduce((best, value) => (
        PUBLIC_WORKFLOW_STATUS_RANK[value] > PUBLIC_WORKFLOW_STATUS_RANK[best] ? value : best
    ), normalizedValues[0]);
}

function uniqueNumbers(values) {
    return [...new Set(
        (Array.isArray(values) ? values : [])
            .map((value) => Number(value))
            .filter((value) => Number.isFinite(value) && value > 0)
    )];
}

function createCheckoutError(code, message, details = null) {
    const error = new Error(message || code);
    error.code = code;
    if (details) {
        error.details = details;
    }
    return error;
}

function buildCheckoutCustomerSnapshot(customer = {}) {
    const customerType = normalizeCustomerType(
        customer.customerType || customer.tipo_cliente || customer.tipoCliente,
        customer
    );
    const country = resolveCheckoutCountryCode(customer);

    return {
        nome: normalizeText(customer.nome),
        email: normalizeText(customer.email).toLowerCase(),
        telefone: normalizeCheckoutPhoneNumber(customer.telefone),
        tipo_cliente: customerType,
        empresa: normalizeText(customer.empresa),
        nif: normalizeTaxId(customer.nif),
        morada: normalizeText(customer.morada),
        codigo_postal: normalizeCheckoutPostalCode(
            customer.codigo_postal,
            country
        ),
        cidade: normalizeText(customer.cidade),
        country
    };
}

function resolveStoredCheckoutCustomer(order = {}, meta = {}, fallbackCustomer = {}) {
    const checkoutPayload = order?.checkout_payload && typeof order.checkout_payload === 'object'
        ? order.checkout_payload
        : {};
    const frozenSources = [
        checkoutPayload?.customer,
        checkoutPayload?.checkoutCustomer,
        meta?.customer,
        meta?.checkoutCustomer
    ].filter((entry) => entry && typeof entry === 'object');
    const frozenCustomer = frozenSources.reduce((acc, source) => ({ ...acc, ...source }), {});
    const fiscalSnapshot = resolveStoredFiscalSnapshot(order, meta);
    const snapshotCustomerType = normalizeCustomerType(fiscalSnapshot.customer_type, frozenCustomer || {});
    const fallback = fallbackCustomer && typeof fallbackCustomer === 'object' ? fallbackCustomer : {};

    return buildCheckoutCustomerSnapshot({
        nome: frozenCustomer.nome
            || frozenCustomer.name
            || fiscalSnapshot.customer_fiscal_name
            || fallback.nome
            || fallback.name,
        email: frozenCustomer.email || fallback.email,
        telefone: frozenCustomer.telefone || frozenCustomer.phone || fallback.telefone || fallback.phone,
        tipo_cliente: frozenCustomer.tipo_cliente
            || frozenCustomer.tipoCliente
            || frozenCustomer.customerType
            || fiscalSnapshot.customer_type
            || fallback.tipo_cliente
            || fallback.tipoCliente
            || fallback.customerType
            || fallback.type,
        empresa: frozenCustomer.empresa
            || (snapshotCustomerType === 'empresa' ? fiscalSnapshot.customer_fiscal_name : '')
            || fallback.empresa,
        nif: frozenCustomer.nif
            || frozenCustomer.vat_number
            || frozenCustomer.vatNumber
            || fiscalSnapshot.vat_validation_number
            || fallback.nif
            || fallback.vat_number
            || fallback.vatNumber,
        morada: frozenCustomer.morada || frozenCustomer.address || fallback.morada || fallback.address,
        codigo_postal: frozenCustomer.codigo_postal
            || frozenCustomer.postalCode
            || frozenCustomer.postal_code
            || fallback.codigo_postal
            || fallback.postalCode
            || fallback.postal_code,
        cidade: frozenCustomer.cidade || frozenCustomer.city || fallback.cidade || fallback.city,
        country: frozenCustomer.country
            || frozenCustomer.countryCode
            || frozenCustomer.country_code
            || fiscalSnapshot.customer_fiscal_country
            || fallback.country
            || fallback.countryCode
            || fallback.country_code
    });
}

function resolveCustomerCountry(customer = {}, customerType = '') {
    const explicit = normalizeCountryCode(
        customer?.country
        || customer?.countryCode
        || customer?.country_code
        || customer?.pais
    );
    if (explicit) {
        return explicit;
    }

    const normalizedPostalCode = String(customer?.codigo_postal || '').trim();
    const normalizedTaxId = normalizeTaxId(customer?.nif);

    if (/^\d{5}(-\d{4})?$/.test(normalizedPostalCode)) {
        return 'ES';
    }

    if (/^\d{4}-?\d{3}$/.test(normalizedPostalCode)) {
        return 'PT';
    }

    if (/^[XYZ]\d{7}[A-Z]$/.test(normalizedTaxId) || /^[A-Z]\d{7}[A-Z0-9]$/.test(normalizedTaxId)) {
        return 'ES';
    }

    if (String(customerType || '').trim().toLowerCase() === 'empresa' && normalizedTaxId && /^[A-Z]/.test(normalizedTaxId)) {
        return 'ES';
    }

    return 'PT';
}

function normalizeCheckoutPhoneNumber(value) {
    const raw = String(value || '').trim();
    if (!raw) {
        return '';
    }

    let normalized = raw.replace(/[^\d+]/g, '');
    if (normalized.startsWith('00')) {
        normalized = `+${normalized.slice(2)}`;
    }

    if (normalized.startsWith('+')) {
        return `+${normalized.slice(1).replace(/\D/g, '')}`;
    }

    return normalized.replace(/\D/g, '');
}

function validateCheckoutCustomerPhone(customer = {}) {
    const snapshot = buildCheckoutCustomerSnapshot(customer);
    const normalized = snapshot.telefone;
    const country = resolveCustomerCountry(snapshot, snapshot.tipo_cliente);

    if (!normalized) {
        return {
            valid: false,
            normalized,
            country,
            message: 'Introduza um numero de telemovel ou telefone valido.'
        };
    }

    const digits = normalized.replace(/^\+/, '');

    if (digits.startsWith('351')) {
        const localNumber = digits.slice(3);
        return {
            valid: /^[29]\d{8}$/.test(localNumber),
            normalized: `+${digits}`,
            country: 'PT',
            message: /^[29]\d{8}$/.test(localNumber)
                ? ''
                : 'O numero de contacto portugues parece invalido.'
        };
    }

    if (digits.startsWith('34')) {
        const localNumber = digits.slice(2);
        return {
            valid: /^[6789]\d{8}$/.test(localNumber),
            normalized: `+${digits}`,
            country: 'ES',
            message: /^[6789]\d{8}$/.test(localNumber)
                ? ''
                : 'O numero de contacto espanhol parece invalido.'
        };
    }

    if (country === 'ES' || /^[678]\d{8}$/.test(digits)) {
        return {
            valid: /^[6789]\d{8}$/.test(digits),
            normalized: `+34${digits}`,
            country: 'ES',
            message: /^[6789]\d{8}$/.test(digits)
                ? ''
                : 'Introduza um numero espanhol com 9 digitos valido.'
        };
    }

    if (country !== 'PT' && country !== 'ES') {
        const validInternational = /^\d{6,15}$/.test(digits);
        return {
            valid: validInternational,
            normalized: normalized.startsWith('+') ? normalized : `+${digits}`,
            country,
            message: validInternational
                ? ''
                : 'Introduza um numero internacional valido com indicativo.'
        };
    }

    return {
        valid: /^[29]\d{8}$/.test(digits),
        normalized: `+351${digits}`,
        country: 'PT',
        message: /^[29]\d{8}$/.test(digits)
            ? ''
            : 'Introduza um numero portugues com 9 digitos valido.'
    };
}

function normalizeCheckoutPostalCode(value, country = 'PT') {
    const normalized = String(value || '').trim().toUpperCase().replace(/\s+/g, '');
    if (!normalized) {
        return '';
    }

    if (String(country || '').toUpperCase() === 'ES') {
        return normalized.replace(/[^\d]/g, '').slice(0, 5);
    }

    if (String(country || '').toUpperCase() !== 'PT') {
        return normalized.slice(0, 16);
    }

    const digits = normalized.replace(/[^\d]/g, '').slice(0, 7);
    if (digits.length === 7) {
        return `${digits.slice(0, 4)}-${digits.slice(4)}`;
    }

    return normalized;
}

function validateCheckoutCustomerType(customer = {}) {
    const snapshot = buildCheckoutCustomerSnapshot(customer);

    if (!['particular', 'empresa'].includes(snapshot.tipo_cliente)) {
        return {
            valid: false,
            normalized: snapshot.tipo_cliente,
            message: 'Escolha se a faturacao e para particular ou empresa.'
        };
    }

    return {
        valid: true,
        normalized: snapshot.tipo_cliente,
        message: ''
    };
}

function validateCheckoutPostalCode(customer = {}) {
    const snapshot = buildCheckoutCustomerSnapshot(customer);
    const country = resolveCustomerCountry(snapshot, snapshot.tipo_cliente);
    const normalized = normalizeCheckoutPostalCode(snapshot.codigo_postal, country);
    const valid = country === 'ES'
        ? /^\d{5}$/.test(normalized)
        : country === 'PT'
            ? /^\d{4}-\d{3}$/.test(normalized)
            : normalized.length >= 3;

    return {
        valid,
        normalized,
        country,
        message: valid
            ? ''
            : country === 'ES'
                ? 'Introduza um codigo postal espanhol com 5 digitos.'
                : country === 'PT'
                    ? 'Introduza um codigo postal portugues no formato 0000-000.'
                    : 'Introduza um codigo postal valido para o pais fiscal selecionado.'
    };
}

function buildCustomerPersistencePayload(customerSnapshot = {}) {
    return {
        nome: customerSnapshot.nome,
        email: customerSnapshot.email,
        telefone: customerSnapshot.telefone,
        empresa: customerSnapshot.empresa,
        nif: customerSnapshot.nif,
        morada: customerSnapshot.morada,
        codigo_postal: customerSnapshot.codigo_postal,
        cidade: customerSnapshot.cidade
    };
}

function validateCheckoutCustomerEmail(email) {
    const normalized = normalizeText(email).toLowerCase();
    if (!normalized || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalized)) {
        return {
            valid: false,
            normalized,
            message: 'Introduza um email valido.'
        };
    }

    const domain = normalized.split('@').pop();
    const suggestedDomain = COMMON_EMAIL_DOMAIN_FIXES[domain];
    if (suggestedDomain) {
        return {
            valid: false,
            normalized,
            suggestion: normalized.replace(/@[^@]+$/, `@${suggestedDomain}`),
            message: `O dominio do email parece invalido: @${domain}. Queria dizer @${suggestedDomain}?`
        };
    }

    return {
        valid: true,
        normalized,
        message: ''
    };
}

function validateCheckoutCustomerTaxId(customer = {}) {
    const snapshot = buildCheckoutCustomerSnapshot(customer);
    return validateTaxId(snapshot.nif, {
        postalCode: snapshot.codigo_postal,
        country: customer.country || customer.pais || ''
    });
}

function hasCustomerIdentityMismatch(existingCustomer = {}, customerSnapshot = {}) {
    const existingEmail = normalizeText(existingCustomer.email).toLowerCase();
    const incomingEmail = normalizeText(customerSnapshot.email).toLowerCase();
    if (existingEmail && incomingEmail && existingEmail !== incomingEmail) {
        return true;
    }

    const existingBusinessName = normalizeName(existingCustomer.empresa || existingCustomer.nome);
    const incomingBusinessName = normalizeName(customerSnapshot.empresa || customerSnapshot.nome);
    if (existingBusinessName && incomingBusinessName && existingBusinessName !== incomingBusinessName) {
        return true;
    }

    return false;
}

function buildCustomerUpdatePayload(existingCustomer = {}, customerSnapshot = {}, strategy = 'overwrite_non_empty') {
    const source = buildCustomerPersistencePayload(customerSnapshot);
    const existing = existingCustomer && typeof existingCustomer === 'object' ? existingCustomer : {};
    const entries = Object.entries(source).filter(([, value]) => String(value || '').trim());

    if (strategy === 'fill_empty_only') {
        return Object.fromEntries(
            entries.filter(([key]) => !String(existing[key] || '').trim())
        );
    }

    return Object.fromEntries(entries);
}

async function findOrCreateCheckoutCustomer(supabase, customer) {
    const customerSnapshot = buildCheckoutCustomerSnapshot(customer);
    if (!customerSnapshot.email) {
        throw createCheckoutError('EMAIL_REQUIRED', 'Email do cliente obrigatorio.');
    }

    const emailValidation = validateCheckoutCustomerEmail(customerSnapshot.email);
    if (!emailValidation.valid) {
        throw createCheckoutError('EMAIL_INVALIDO', emailValidation.message, {
            field: 'email',
            suggestion: emailValidation.suggestion || null
        });
    }

    const phoneValidation = validateCheckoutCustomerPhone(customerSnapshot);
    if (!phoneValidation.valid) {
        throw createCheckoutError('TELEFONE_INVALIDO', phoneValidation.message, {
            field: 'telefone'
        });
    }
    customerSnapshot.telefone = phoneValidation.normalized;

    const customerTypeValidation = validateCheckoutCustomerType(customerSnapshot);
    if (!customerTypeValidation.valid) {
        throw createCheckoutError('TIPO_CLIENTE_INVALIDO', customerTypeValidation.message, {
            field: 'tipo_cliente'
        });
    }

    const postalCodeValidation = validateCheckoutPostalCode(customerSnapshot);
    if (!postalCodeValidation.valid) {
        throw createCheckoutError('CODIGO_POSTAL_INVALIDO', postalCodeValidation.message, {
            field: 'codigo_postal'
        });
    }
    customerSnapshot.codigo_postal = postalCodeValidation.normalized;

    if (customerSnapshot.tipo_cliente === 'empresa') {
        if (!customerSnapshot.empresa) {
            throw createCheckoutError('EMPRESA_REQUIRED', 'Indique o nome fiscal da empresa para emitir a fatura.', {
                field: 'empresa'
            });
        }
    }

    const selectFields = 'id,email,nome,telefone,empresa,nif,morada,codigo_postal,cidade,created_at';
    let existingCustomer = null;
    let matchedBy = '';

    const { data: emailMatches, error: emailLookupError } = await supabase
        .from('clientes')
        .select(selectFields)
        .eq('email', customerSnapshot.email)
        .order('created_at', { ascending: false })
        .limit(1);

    if (emailLookupError) {
        throw emailLookupError;
    }

    if (Array.isArray(emailMatches) && emailMatches.length > 0) {
        existingCustomer = emailMatches[0];
        matchedBy = 'email';
    }

    if (!existingCustomer && customerSnapshot.telefone) {
        const { data: phoneMatches, error: phoneLookupError } = await supabase
            .from('clientes')
            .select(selectFields)
            .eq('telefone', customerSnapshot.telefone)
            .order('created_at', { ascending: false })
            .limit(1);

        if (phoneLookupError) {
            throw phoneLookupError;
        }

        if (Array.isArray(phoneMatches) && phoneMatches.length > 0) {
            existingCustomer = phoneMatches[0];
            matchedBy = 'telefone';
        }
    }

    const matchedCustomer = existingCustomer || null;

    if (matchedCustomer?.id) {
        const updatePayload = buildCustomerUpdatePayload(
            matchedCustomer,
            customerSnapshot,
            matchedBy === 'tax_id' ? 'fill_empty_only' : 'overwrite_non_empty'
        );

        if (Object.keys(updatePayload).length > 0) {
            const { error: updateError } = await supabase
                .from('clientes')
                .update(updatePayload)
                .eq('id', matchedCustomer.id);

            if (updateError) {
                throw updateError;
            }
        }

        return matchedCustomer.id;
    }

    const { data: createdCustomer, error: createError } = await supabase
        .from('clientes')
        .insert([buildCustomerPersistencePayload(customerSnapshot)])
        .select('id')
        .single();

    if (createError) {
        throw createError;
    }

    return createdCustomer?.id || null;
}

async function fetchProductsForCheckout(supabase, rawItems) {
    const productIds = uniqueNumbers(rawItems.map((item) => item?.id || item?.produtoId || item?.produto_id));
    const productRows = [];
    const productIdsSeen = new Set();

    if (productIds.length > 0) {
        const { data, error } = await supabase
            .from('produtos')
            .select('id,nome,preco,imagem,categoria,ativo')
            .in('id', productIds);

        if (error) {
            throw error;
        }

        (data || []).forEach((product) => {
            productRows.push(product);
            productIdsSeen.add(Number(product.id));
        });
    }

    const hasNameOnlyItems = rawItems.some((item) => !Number(item?.id || item?.produtoId || item?.produto_id) && normalizeName(item?.nome));
    if (hasNameOnlyItems || productRows.length < productIds.length) {
        const { data, error } = await supabase
            .from('produtos')
            .select('id,nome,preco,imagem,categoria,ativo');

        if (error) {
            throw error;
        }

        (data || []).forEach((product) => {
            const id = Number(product.id);
            if (!productIdsSeen.has(id)) {
                productRows.push(product);
                productIdsSeen.add(id);
            }
        });
    }

    return productRows;
}

function getFlyBannerCatalogBaseSlugs() {
    return [
        'flybanner-cruzeta-com-flutuador',
        'flybanner-base-parede',
        'flybanner-base-parafuso-roscado',
        'flybanner-base-pica',
        'flybanner-base-hercules-12kg',
        'flybanner-base-agua',
        'flybanner-base-deluxe-4kg',
        'flybanner-base-universal-com-abracadeiras',
        'flybanner-base-distancia-entre-eixos-do-carro',
        'flybanner-base-para-tenda'
    ];
}

function isFlyBannerCatalogBaseSlug(slug) {
    const normalizedSlug = normalizeText(slug).toLowerCase();
    if (!normalizedSlug) {
        return false;
    }

    return getFlyBannerCatalogBaseSlugs().includes(normalizedSlug)
        || normalizedSlug.startsWith('flybanner-base-custom-');
}

function buildBaseRecordFromCatalog(productId, base, assignment = null, fallbackOrder = 0) {
    return {
        produto_id: Number(productId),
        base_id: Number(base?.id),
        base_nome: normalizeText(base?.nome),
        base_nome_es: normalizeText(base?.nome_es),
        base_imagem: normalizeText(base?.imagem),
        preco_extra_aplicado: Number(assignment?.preco_extra_override ?? base?.preco_extra ?? 0) || 0,
        is_default: assignment
            ? (assignment?.is_default === true || String(assignment?.is_default) === 'true')
            : fallbackOrder === 0,
        ativo: assignment?.ativo !== false,
        base_ativa: base?.ativo !== false,
        base_disponivel: base?.disponivel !== false && String(base?.disponivel) !== 'false',
        base_nota_indisponibilidade: normalizeText(base?.nota_indisponibilidade)
    };
}

async function fetchProductBasesForCheckout(supabase, products) {
    const productRows = Array.isArray(products) ? products : [];
    const productIds = uniqueNumbers(productRows.map((product) => product?.id));
    if (productIds.length === 0) {
        return [];
    }

    try {
        const { data: assignments, error: assignmentsError } = await supabase
            .from('produto_bases_fixacao')
            .select('produto_id,base_id,ativo,ordem,is_default,preco_extra_override')
            .in('produto_id', productIds)
            .eq('ativo', true)
            .order('ordem', { ascending: true });

        if (assignmentsError) {
            throw assignmentsError;
        }

        const assignmentRows = Array.isArray(assignments) ? assignments : [];
        const assignedProductIds = new Set(
            assignmentRows
                .map((row) => Number(row?.produto_id))
                .filter(Number.isFinite)
        );
        const baseIds = uniqueNumbers(assignmentRows.map((row) => row?.base_id));
        const flyBannerProductsWithoutAssignments = productRows.filter((product) => (
            isFlyBannerProductCategory(product?.categoria)
            && !assignedProductIds.has(Number(product?.id))
        ));

        const catalogBaseIds = [...baseIds];
        let globalFlyBannerBases = [];

        if (flyBannerProductsWithoutAssignments.length > 0) {
            const { data: globalBaseData, error: globalBasesError } = await supabase
                .from('bases_fixacao')
                .select('id,nome,nome_es,slug,imagem,preco_extra,ativo,disponivel,nota_indisponibilidade,ordem')
                .eq('ativo', true)
                .order('ordem', { ascending: true });

            if (globalBasesError) {
                throw globalBasesError;
            }

            globalFlyBannerBases = (Array.isArray(globalBaseData) ? globalBaseData : [])
                .filter((base) => isFlyBannerCatalogBaseSlug(base?.slug));
            globalFlyBannerBases.forEach((base) => {
                const baseId = Number(base?.id);
                if (Number.isFinite(baseId) && !catalogBaseIds.includes(baseId)) {
                    catalogBaseIds.push(baseId);
                }
            });
        }

        const { data: basesData, error: basesError } = await supabase
            .from('bases_fixacao')
            .select('id,nome,nome_es,slug,imagem,preco_extra,ativo,disponivel,nota_indisponibilidade,ordem')
            .in('id', catalogBaseIds.length > 0 ? catalogBaseIds : [-1]);

        if (basesError) {
            throw basesError;
        }

        const baseMap = new Map(
            (Array.isArray(basesData) ? basesData : [])
                .map((base) => [Number(base?.id), base])
        );

        const assignedBaseRows = assignmentRows.map((assignment) => {
            const baseId = Number(assignment?.base_id);
            const base = baseMap.get(baseId);
            if (!Number.isFinite(baseId) || !base) {
                return null;
            }

            return buildBaseRecordFromCatalog(assignment?.produto_id, base, assignment);
        }).filter(Boolean);

        const globalFallbackRows = flyBannerProductsWithoutAssignments.flatMap((product) => {
            const productId = Number(product?.id);
            return globalFlyBannerBases.map((base, index) => buildBaseRecordFromCatalog(productId, base, null, index));
        });

        if (assignedBaseRows.length > 0 || globalFallbackRows.length > 0) {
            return [...assignedBaseRows, ...globalFallbackRows];
        }
    } catch (error) {
        const raw = String(error?.message || '').toLowerCase();
        if (
            error?.code !== 'PGRST205'
            && !raw.includes('produto_bases_fixacao')
            && !raw.includes('bases_fixacao')
        ) {
            throw error;
        }
    }

    try {
        const { data, error } = await supabase
            .from('vw_produto_bases')
            .select('produto_id,base_id,base_nome,base_imagem,preco_extra_aplicado,is_default,ativo,base_ativa,base_disponivel,base_nota_indisponibilidade')
            .in('produto_id', productIds);

        if (error) {
            throw error;
        }

        return Array.isArray(data) ? data : [];
    } catch (error) {
        const raw = String(error?.message || '').toLowerCase();
        if (error?.code === 'PGRST205' || raw.includes('vw_produto_bases')) {
            return [];
        }
        throw error;
    }
}

function isFlyBannerProductCategory(category) {
    const normalized = normalizeText(category).toLowerCase();
    return normalized === 'fly-banner' || normalized === 'flybanners';
}

function isSelectableBaseRecord(base) {
    return base?.ativo !== false
        && base?.base_ativa !== false
        && base?.base_disponivel !== false
        && String(base?.base_disponivel) !== 'false';
}

function buildProductLookup(products) {
    const byId = new Map();
    const byName = new Map();

    (products || []).forEach((product) => {
        const id = Number(product?.id);
        if (Number.isFinite(id)) {
            byId.set(id, product);
        }

        const name = normalizeName(product?.nome);
        if (name && !byName.has(name)) {
            byName.set(name, product);
        }
    });

    return { byId, byName };
}

function buildBaseLookup(bases) {
    const byProductAndBaseId = new Map();
    const byProductId = new Map();
    const byProductIdSelectable = new Map();

    (bases || []).forEach((base) => {
        if (base?.ativo === false || base?.base_ativa === false) {
            return;
        }

        const productId = Number(base?.produto_id);
        const baseId = Number(base?.base_id);
        if (Number.isFinite(productId) && Number.isFinite(baseId)) {
            byProductAndBaseId.set(`${productId}:${baseId}`, base);
            if (!byProductId.has(productId)) {
                byProductId.set(productId, []);
            }
            byProductId.get(productId).push(base);

            if (isSelectableBaseRecord(base)) {
                if (!byProductIdSelectable.has(productId)) {
                    byProductIdSelectable.set(productId, []);
                }
                byProductIdSelectable.get(productId).push(base);
            }
        }
    });

    return { byProductAndBaseId, byProductId, byProductIdSelectable };
}

function resolveProductForItem(item, productLookup) {
    const candidateId = Number(item?.id || item?.produtoId || item?.produto_id);
    if (Number.isFinite(candidateId) && productLookup.byId.has(candidateId)) {
        return productLookup.byId.get(candidateId);
    }

    const candidateName = normalizeName(item?.nome);
    if (candidateName && productLookup.byName.has(candidateName)) {
        return productLookup.byName.get(candidateName);
    }

    throw createCheckoutError(
        'MISSING_PRODUCT_MAPPING',
        `Produto nao encontrado na base de dados: ${normalizeText(item?.nome) || candidateId || 'desconhecido'}`
    );
}

function normalizeDesignPayload(value, maxLength = 750000) {
    const text = normalizeText(value);
    if (!text) {
        return '';
    }
    return text.slice(0, maxLength);
}

async function fetchDesignSnapshotsForCheckout(supabase, rawItems) {
    const designIds = [...new Set(
        (Array.isArray(rawItems) ? rawItems : [])
            .map((item) => normalizeText(item?.designId || item?.design_id))
            .filter(Boolean)
    )];

    if (designIds.length === 0) {
        return new Map();
    }

    try {
        const { data, error } = await supabase
            .from('design_snapshots')
            .select('design_id, design_preview, storage_bucket, masked_svg_path, document_path, asset_manifest')
            .in('design_id', designIds);

        if (error) {
            throw error;
        }

        const map = new Map();
        (Array.isArray(data) ? data : []).forEach((row) => {
            const id = normalizeText(row?.design_id);
            if (!id) return;

            map.set(id, {
                designPreview: normalizeDesignPayload(row?.design_preview, 1_500_000),
                storageBucket: normalizeText(row?.storage_bucket),
                maskedSvgPath: normalizeText(row?.masked_svg_path),
                documentPath: normalizeText(row?.document_path),
                assetManifest: row?.asset_manifest && typeof row.asset_manifest === 'object'
                    ? row.asset_manifest
                    : {}
            });
        });

        return map;
    } catch (error) {
        // Fallback silencioso para não bloquear checkout caso a tabela não exista/esteja indisponível.
        return new Map();
    }
}

function buildResolvedCheckoutItem(item, product, baseLookup, designSnapshot = null) {
    if (product?.ativo === false) {
        throw createCheckoutError('PRODUCT_INACTIVE', `Produto indisponivel: ${product.nome || product.id}`);
    }

    const productId = Number(product.id);
    const baseId = Number(item?.baseId || item?.base_id || 0);
    const base = baseId > 0
        ? baseLookup.byProductAndBaseId.get(`${productId}:${baseId}`)
        : null;
    const requiresBaseSelection = isFlyBannerProductCategory(product?.categoria);
    const configuredBases = baseLookup.byProductId.get(productId) || [];
    const selectableBases = baseLookup.byProductIdSelectable.get(productId) || [];

    if (requiresBaseSelection && configuredBases.length === 0) {
        throw createCheckoutError('BASE_CONFIG_MISSING', `O produto ${product.nome || productId} nao tem bases de fixacao configuradas.`);
    }

    if (requiresBaseSelection && selectableBases.length === 0) {
        throw createCheckoutError('BASE_UNAVAILABLE', `O produto ${product.nome || productId} nao tem bases de fixacao disponiveis.`);
    }

    if (requiresBaseSelection && !(baseId > 0)) {
        throw createCheckoutError('BASE_REQUIRED', `Selecione uma base de fixacao para o produto ${product.nome || productId}.`);
    }

    if (baseId > 0 && !base) {
        throw createCheckoutError('BASE_INVALIDA', `Base invalida para o produto ${product.nome || productId}.`);
    }

    if (base && !isSelectableBaseRecord(base)) {
        throw createCheckoutError('BASE_INVALIDA', `Base invalida para o produto ${product.nome || productId}.`);
    }

    const productPrice = toNumber(product.preco, 0);
    const baseExtra = base ? toNumber(base.preco_extra_aplicado, 0) : 0;
    const unitPrice = Math.max(0, Number((productPrice + baseExtra).toFixed(2)));
    const quantity = normalizeQuantity(item?.quantity || item?.quantidade);
    const snapshotDesignPreview = normalizeDesignPayload(designSnapshot?.designPreview, 1_500_000);
    const incomingDesignPreview = normalizeDesignPayload(item?.designPreview || item?.design_preview, 1_500_000);
    const designId = normalizeText(item?.designId || item?.design_id);
    const designReadToken = normalizeText(item?.designReadToken || item?.design_read_token);
    const designSvgUrl = normalizeText(item?.designSvgUrl || item?.design_svg_url)
        || (designId && designReadToken ? buildDesignAssetUrl(designId, designReadToken, 'svg') : '');

    if (unitPrice <= 0) {
        throw createCheckoutError('TOTAL_INVALIDO', `Preco invalido para o produto ${product.nome || productId}.`);
    }

    return {
        id: productId,
        produtoId: productId,
        nome: normalizeText(product.nome) || normalizeText(item?.nome) || 'Produto',
        quantity,
        quantidade: quantity,
        preco: unitPrice,
        precoUnitario: unitPrice,
        imagem: normalizeText(product.imagem || item?.imagem),
        customized: Boolean(item?.customized),
        designId,
        designReadToken,
        designSvgUrl,
        designStorageBucket: normalizeText(item?.designStorageBucket || item?.design_storage_bucket) || normalizeText(designSnapshot?.storageBucket),
        designStoragePath: normalizeText(item?.designStoragePath || item?.design_storage_path) || normalizeText(designSnapshot?.maskedSvgPath),
        designDocumentPath: normalizeText(designSnapshot?.documentPath),
        designAssetManifest: designSnapshot?.assetManifest || {},
        design: normalizeDesignPayload(item?.design),
        designPreview: designSvgUrl || snapshotDesignPreview || incomingDesignPreview,
        designSceneV1: item?.designSceneV1 && typeof item.designSceneV1 === 'object'
            ? item.designSceneV1
            : (item?.design_scene_v1 && typeof item.design_scene_v1 === 'object' ? item.design_scene_v1 : null),
        baseId: base ? Number(base.base_id) : null,
        baseNome: base ? normalizeText(base.base_nome) : '',
        baseImagem: base ? normalizeText(base.base_imagem) : '',
        basePrecoExtra: Number(baseExtra.toFixed(2))
    };
}

async function resolveCheckoutCart(supabase, rawItems) {
    const items = Array.isArray(rawItems) ? rawItems : [];
    if (items.length === 0) {
        throw createCheckoutError('CARRINHO_VAZIO', 'O carrinho esta vazio.');
    }

    const products = await fetchProductsForCheckout(supabase, items);
    const productLookup = buildProductLookup(products);

    const resolvedProducts = items.map((item) => resolveProductForItem(item, productLookup));
    const productIds = uniqueNumbers(resolvedProducts.map((product) => product.id));
    const bases = await fetchProductBasesForCheckout(supabase, resolvedProducts);
    const baseLookup = buildBaseLookup(bases);
    const designSnapshotsById = await fetchDesignSnapshotsForCheckout(supabase, items);

    return items.map((item, index) => {
        const designId = normalizeText(item?.designId || item?.design_id);
        const designSnapshot = designId ? designSnapshotsById.get(designId) || null : null;
        return buildResolvedCheckoutItem(item, resolvedProducts[index], baseLookup, designSnapshot);
    });
}

function calculateCheckoutTotals(items) {
    const subtotal = (Array.isArray(items) ? items : []).reduce((sum, item) => {
        return sum + (
            toNumber(item.preco ?? item.precoUnitario, 0)
            * normalizeQuantity(item.quantity ?? item.quantidade)
        );
    }, 0);
    const shipping = 0;
    const total = subtotal + shipping;

    return {
        subtotal: Number(subtotal.toFixed(2)),
        shipping,
        total: Number(total.toFixed(2))
    };
}

function buildInitialOrderMeta(customerSnapshot, cart, selectedPaymentMethod, notes, serviceOptions = {}) {
    const normalizedServiceOptions = normalizeCheckoutServiceOptions(serviceOptions);
    const chargeableItems = [
        ...(Array.isArray(cart) ? cart : []),
        ...buildServiceOptionItems(normalizedServiceOptions)
    ];
    const itemSnapshots = buildOrderItemSnapshots(chargeableItems);

    return {
        workflowStatus: 'em_preparacao',
        paymentStatus: 'pending',
        paymentProvider: 'stripe',
        paymentMethod: normalizePaymentMethodType(selectedPaymentMethod),
        trackingCode: '',
        trackingUrl: '',
        stripeSessionId: '',
        stripePaymentIntent: '',
        facturalusaCustomerCode: '',
        facturalusaDocumentNumber: '',
        facturalusaDocumentUrl: '',
        facturalusaLastError: '',
        facturalusaStatus: 'pending',
        facturalusaLastAttemptAt: '',
        fiscalSnapshot: normalizeFiscalSnapshot({}),
        vatValidation: {},
        serviceOptions: normalizedServiceOptions,
        fiscalDivergence: {
            diverged: false,
            fields: [],
            reason: ''
        },
        statusHistory: [
            {
                status: 'em_preparacao',
                at: new Date().toISOString(),
                note: 'Encomenda criada no checkout'
            }
        ],
        itemSnapshots,
        checkoutCustomer: customerSnapshot,
        checkoutSnapshot: buildCheckoutPayloadSnapshot(customerSnapshot, cart, selectedPaymentMethod, notes, normalizedServiceOptions)
    };
}

function buildOrderItemsFromSnapshots(meta) {
    const sourceMeta = meta && typeof meta === 'object' ? meta : {};
    const snapshots = Array.isArray(sourceMeta.itemSnapshots) ? sourceMeta.itemSnapshots : [];

    return snapshots.map((snapshot) => ({
        produto_id: snapshot.produtoId || null,
        quantidade: snapshot.quantidade || 1,
        preco_unitario: snapshot.precoUnitario || 0,
        subtotal: (Number(snapshot.precoUnitario || 0) * Number(snapshot.quantidade || 1)),
        design_id: snapshot.designId || null,
        design_preview: snapshot.designPreview || null,
        design_svg_url: snapshot.designSvgUrl || snapshot.designPreview || null,
        designSvgUrl: snapshot.designSvgUrl || snapshot.designPreview || null,
        design_storage_bucket: snapshot.designStorageBucket || null,
        designStorageBucket: snapshot.designStorageBucket || null,
        design_storage_path: snapshot.designStoragePath || null,
        designStoragePath: snapshot.designStoragePath || null,
        nome_produto: snapshot.nome || 'Produto',
        imagem_produto: snapshot.imagem || '',
        base_id: snapshot.baseId || null,
        base_nome: snapshot.baseNome || '',
        base_preco_extra: snapshot.basePrecoExtra || 0,
        produtos: {
            id: snapshot.produtoId || null,
            nome: snapshot.nome || 'Produto',
            imagem: snapshot.imagem || '',
            preco: snapshot.precoUnitario || 0
        }
    }));
}

function buildPublicOrderPayload(order, meta) {
    const paymentStatus = normalizeText(order?.payment_status) || normalizeText(meta?.paymentStatus) || 'pending';
    const paymentProvider = normalizeText(order?.payment_provider) || normalizeText(meta?.paymentProvider) || 'stripe';
    const paymentMethod = normalizeText(order?.metodo_pagamento) || normalizeText(meta?.paymentMethod);
    const invoiceState = resolveFacturalusaDocumentState(order, meta);
    const workflowStatus = pickMostAdvancedPublicWorkflowStatus(
        meta?.workflowStatus,
        resolveWorkflowStatusFromPublicOrderRow(order)
    );
    const fiscalSnapshot = normalizeFiscalSnapshot(
        order?.fiscal_snapshot
        || order?.checkout_payload?.fiscalSnapshot
        || meta?.fiscalSnapshot
        || {}
    );
    const fiscalDivergence = meta?.fiscalDivergence && typeof meta.fiscalDivergence === 'object'
        ? meta.fiscalDivergence
        : { diverged: false, fields: [], reason: '' };

    return {
        id: order?.id || null,
        numero_encomenda: order?.numero_encomenda || null,
        status: order?.status || null,
        workflow_status: workflowStatus,
        workflowStatus,
        subtotal: order?.subtotal || 0,
        envio: order?.envio || 0,
        total: order?.total || 0,
        metodo_pagamento: paymentMethod,
        payment_provider: paymentProvider,
        payment_status: paymentStatus,
        facturalusa_status: invoiceState.status,
        facturalusa_document_number: invoiceState.documentNumber,
        facturalusa_document_url: invoiceState.documentUrl,
        invoice_state: normalizeText(order?.invoice_state) || fiscalSnapshot.invoice_state || invoiceState.status,
        fiscal_scenario: normalizeText(order?.fiscal_scenario) || fiscalSnapshot.fiscal_scenario || '',
        fiscal_review_required: (normalizeText(order?.invoice_state) || fiscalSnapshot.invoice_state) === 'pending_manual_review',
        vat_validation_status: normalizeText(order?.vat_validation_status) || fiscalSnapshot.vat_validation_status || '',
        vat_validation_number: normalizeText(order?.vat_validation_number) || fiscalSnapshot.vat_validation_number || '',
        customer_fiscal_country: normalizeText(order?.customer_fiscal_country) || fiscalSnapshot.customer_fiscal_country || '',
        fiscal_divergence: fiscalDivergence,
        created_at: order?.created_at || null,
        updated_at: order?.updated_at || null,
        tracking_codigo: meta?.trackingCode || order?.tracking_codigo || order?.codigo_tracking || order?.tracking_code || null,
        tracking_url: meta?.trackingUrl || order?.tracking_url || order?.url_tracking || order?.tracking_link || null
    };
}

function buildPublicOrderItemPayload(item = {}) {
    const product = item?.produtos && typeof item.produtos === 'object' ? item.produtos : {};
    const productName = normalizeText(item.nome_produto || product.nome) || 'Produto';
    const productImage = normalizeText(item.imagem_produto || product.imagem);
    const quantity = normalizeQuantity(item.quantidade || 1);
    const unitPrice = toNumber(item.preco_unitario || product.preco, 0);

    return {
        produto_id: item.produto_id || product.id || null,
        quantidade: quantity,
        preco_unitario: unitPrice,
        subtotal: toNumber(item.subtotal, unitPrice * quantity),
        design_id: normalizeText(item.design_id),
        design_preview: normalizeText(item.design_preview),
        design_svg_url: normalizeText(item.design_svg_url),
        designSvgUrl: normalizeText(item.design_svg_url || item.design_preview),
        designStorageUrl: normalizeText(item.design_svg_url),
        design_storage_bucket: normalizeText(item.design_storage_bucket),
        designStorageBucket: normalizeText(item.design_storage_bucket),
        design_storage_path: normalizeText(item.design_storage_path),
        designStoragePath: normalizeText(item.design_storage_path),
        nome_produto: productName,
        imagem_produto: productImage,
        base_id: item.base_id || null,
        base_nome: normalizeText(item.base_nome),
        base_preco_extra: toNumber(item.base_preco_extra, 0),
        produtos: {
            id: product.id || item.produto_id || null,
            nome: productName,
            imagem: productImage,
            preco: unitPrice,
            svg_template: normalizeText(product.svg_template)
        }
    };
}

function buildPublicOrderItemsFromRows(items) {
    return (Array.isArray(items) ? items : []).map(buildPublicOrderItemPayload);
}

module.exports = {
    buildCheckoutCustomerSnapshot,
    buildInitialOrderMeta,
    buildOrderItemsFromSnapshots,
    buildPublicOrderItemsFromRows,
    buildPublicOrderItemPayload,
    buildPublicOrderPayload,
    calculateCheckoutTotals,
    createCheckoutError,
    findOrCreateCheckoutCustomer,
    normalizeQuantity,
    normalizeCheckoutPhoneNumber,
    normalizeCheckoutPostalCode,
    resolveCheckoutCart,
    resolveStoredCheckoutCustomer,
    validateCheckoutCustomerEmail,
    validateCheckoutCustomerPhone,
    validateCheckoutCustomerType,
    validateCheckoutCustomerTaxId,
    validateCheckoutPostalCode
};
