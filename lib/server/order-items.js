...
async function insertOrderItemsWithFallback(supabase, orderId, items) {
    ...
    const optionalValueByColumn = {
        ...
        design_preview: (item) => item.designPreview || item.design_preview || item.designSvgUrl || item.design_svg_url || null,
        design_svg_url: (item) => item.designSvgUrl || item.design_svg_url || item.designPreview || item.design_preview || null,
        design_storage_bucket: (item) => item.designStorageBucket || item.design_storage_bucket || null,
        design_storage_path: (item) => item.designStoragePath || item.design_storage_path || null,
        nome_produto: (item) => item.nome || null,
        imagem_produto: (item) => item.imagem || null,
        base_id: (item) => item.baseId || item.base_id || null,
        base_nome: (item) => item.baseNome || item.base_nome || null,
        base_preco_extra: (item) => Number(item.basePrecoExtra || item.base_preco_extra || 0)
    };
