...
const FLYBANNER_HEIGHT_NAME_MAP = Object.freeze({
    'fly-banner-surf-290cm': 'Fly Banner Surf 290CM',
    'fly-banner-surf-340cm': 'Fly Banner Surf 340CM',
    'fly-banner-surf-400cm': 'Fly Banner Surf 400CM',
    'fly-banner-surf-500cm': 'Fly Banner Surf 500CM',
    'fly-banner-surf-600cm': 'Fly Banner Surf 600CM',
    'fly-banner-drop-245cm': 'Fly Banner Drop 245CM',
    'fly-banner-drop-300cm': 'Fly Banner Drop 300CM',
    'fly-banner-drop-350cm': 'Fly Banner Drop 350CM',
    'fly-banner-drop-440cm': 'Fly Banner Drop 440CM',
    'fly-banner-drop-540cm': 'Fly Banner Drop 540CM'
});
...
function getAdminSupabaseBootstrapError() {
    if (!window.supabase || typeof window.supabase.createClient !== 'function') {
        return 'SDK do Supabase não foi carregado.';
    }

    if (!window.APP_CONFIG?.SUPABASE_URL || !window.APP_CONFIG?.SUPABASE_ANON_KEY) {
        return 'Configuração do Supabase incompleta em APP_CONFIG.';
    }
...
function ensureAdminSupabaseReady() {
    ...
    setAdminOverlayStatus(`Erro técnico do admin: ${bootstrapError}`, 'error');
...
async function validateAdminSession(session) {
    ...
    if (!accessToken) {
        adminWriteSessionLastError = 'Sessão admin inválida.';
        return false;
    }
    ...
    try {
        ...
        if (!response.ok) {
            adminWriteSessionLastError = payload?.message || payload?.error || 'Utilizador sem permissões admin.';
            return false;
        }
        ...
        adminWriteSessionLastError = error?.message || 'Falha ao validar permissão admin.';
        return false;
    }
}
...
async function ensureAdminWriteSession() {
    ...
    try {
        ...
        adminWriteSessionLastError = session
            ? (adminWriteSessionLastError || 'Sessão autenticada não pertence ao admin autorizado.')
        ...
        adminWriteSessionLastError = error?.message || 'Falha inesperada ao validar sessão.';
        showLoginOverlay();
        return false;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            if (!email) {
                errorEl.textContent = 'O email interno do admin não está configurado.';
            ...
            if (error) {
                ...
                errorEl.textContent = 'Credenciais inválidas. Verifique a password do admin.';
                errorEl.classList.remove('hidden');
                btn.disabled = false;
            ...
            if (!await validateAdminSession(session)) {
                await supabaseClient.auth.signOut();
                errorEl.textContent = adminWriteSessionLastError || 'Acesso não autorizado para este utilizador.';
                errorEl.classList.remove('hidden');
                btn.disabled = false;
                btnText.textContent = 'Entrar';
                showLoginOverlay();
...
function renderDashboardMailHealth(mailHealth) {
    ...
    const fromEmail = String(mailHealth?.fromEmail || '').trim() || 'não configurado';
    const replyTo = String(mailHealth?.replyTo || '').trim() || 'não configurado';
...
function isMissingBasesSchema(error) {
    ...
    return msg.includes('bases_fixacao')
        || msg.includes('produto_bases_fixacao')
        || msg.includes('disponivel')
        || msg.includes('nota_indisponibilidade')
        || msg.includes('base_disponivel')
        || msg.includes('base_nota_indisponibilidade');
}
                ...
                <div class="flex items-start justify-between gap-3">
                    <label class="flex items-start gap-2 cursor-pointer flex-1">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">+${formatCurrency(base.preco_extra || 0)}${base.ativo ? '' : ' - Inativa'}${base.disponivel === false ? ' - Indisponível no cliente' : ''}</p>
                        </div>
                    </label>
    ...
    tbody.innerHTML = items.map((base) => {
        ...
        return `
            <tr>
                ...
                <td>${base.ordem || 0}</td>
                <td>${base.disponivel === false ? '<span class="badge badge-warning">Indisponível</span>' : '<span class="badge badge-success">Disponível</span>'}</td>
...
async function syncFlybannerHeightNames(products = []) {
    ...
    const updateResults = await Promise.all(candidates.map(async (product) => {
        ...
        if (error) {
            console.warn('Não foi possível sincronizar nome Fly Banner:', product?.id, error);
            return { id: product?.id, updated: false };
        }
...
function parseSvgTemplate(svgText) {
    ...
    if (parserError || !root || root.tagName.toLowerCase() !== 'svg') {
        throw new Error('Ficheiro SVG inválido');
    }
...
async function editProduct(id) {
    try {
        ...
        if (modal) {
            ...
            console.error('Modal product-modal não encontrado no DOM');
        }
...
if (addBaseBtn) {
    addBaseBtn.addEventListener('click', () => {
        ...
        document.getElementById('base-disponivel').checked = true;
        document.getElementById('base-nota-indisponibilidade').value = '';
        openModal(baseModal);
    });
}
...
if (baseForm) {
    baseForm.addEventListener('submit', async (e) => {
        ...
        const baseData = {
            ...
            nota_indisponibilidade: document.getElementById('base-nota-indisponibilidade').value.trim() || null
        };
...
async function editBase(id) {
    try {
        ...
        document.getElementById('base-disponivel').checked = data.disponivel !== false;
        document.getElementById('base-nota-indisponibilidade').value = data.nota_indisponibilidade || '';
...
function formatFiscalScenarioLabel(value) {
    switch (String(value || '').trim()) {
        ...
        case 'profile_override_manual_review':
            return 'Perfil fiscal não suportado';
        default:
            return 'Por definir';
    }
}
...
async function viewOrder(id) {
    try {
        ...
        if (!order) {
            showToast('Encomenda não encontrada', 'warning');
            return;
        }
        ...
        try {
            ...
            if (nextWorkflowStatus !== previousWorkflowStatus) {
                try {
                    ...
                    if (emailResult?.sent) {
                        showToast('Email de atualização enviado ao cliente', 'success');
                    } else if (emailResult?.reason && !['DUPLICATE_EMAIL', 'STATUS_EMAIL_NOT_ENABLED'].includes(emailResult.reason)) {
                        showToast(`Email não enviado: ${emailResult.message || emailResult.reason}`, 'warning');
                    }
                } catch (emailError) {
                    console.warn('Encomenda atualizada, mas o email de estado falhou:', emailError);
                    showToast(`Encomenda atualizada. Email não enviado: ${emailError?.message || 'erro de email'}`, 'warning');
                }
            }
