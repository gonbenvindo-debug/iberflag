...
Object.assign(DesignEditor.prototype, {
    ...
    startCropMode() {
        ...
        if (!imgElement || imgElement.tagName !== 'image') {
            showToast(window.personalizerI18nText ? window.personalizerI18nText('Imagem nÃ£o encontrada') : 'Imagem nÃ£o encontrada', 'error');
            return;
        }
        ...
        if (!srcToCrop) {
            showToast(window.personalizerI18nText ? window.personalizerI18nText('Fonte da imagem nÃ£o encontrada') : 'Fonte da imagem nÃ£o encontrada', 'error');
            return;
        }
        ...
        if (this.requiresBaseSelection?.() && !selectedBase) {
            showToast(
                this.hasMissingRequiredBaseConfiguration?.()
                    ? (window.personalizerI18nText ? window.personalizerI18nText('Este Fly Banner não tem bases de fixação configuradas no admin. Configure uma base antes de continuar.') : 'Este Fly Banner não tem bases de fixação configuradas no admin. Configure uma base antes de continuar.')
                    : (window.personalizerI18nText ? window.personalizerI18nText('Selecione uma base de fixação disponível antes de adicionar ao carrinho.') : 'Selecione uma base de fixação disponível antes de adicionar ao carrinho.'),
                'warning'
            );
            return;
        }
