...
function syncProductQuantityLink(input, options = {}) {
    ...
    const quantity = rawValue === '' ? 1 : normalizeProductQuantity(rawValue);

    if (shouldCommit || rawValue !== '') {
...
function initProductQuantityControls() {
    document.querySelectorAll('[data-product-quantity-input]').forEach((input) => {
        ...
        input.addEventListener('input', () => syncProductQuantityLink(input, { commit: false }));
        input.addEventListener('change', () => syncProductQuantityLink(input));
        input.addEventListener('blur', () => syncProductQuantityLink(input));
...
function normalizeCartItem(item) {
    ...
    return {
        ...
        designPreview: item.designPreview || item.designSvgUrl || item.design_svg_url || null,
        designReadToken: item.designReadToken || item.design_read_token || null,
        design_read_token: item.designReadToken || item.design_read_token || null,
        designSvgUrl: item.designSvgUrl || item.design_svg_url || null,
        design_svg_url: item.designSvgUrl || item.design_svg_url || null,
        designStorageBucket: item.designStorageBucket || item.design_storage_bucket || null,
        design_storage_bucket: item.designStorageBucket || item.design_storage_bucket || null,
        designStoragePath: item.designStoragePath || item.design_storage_path || null,
        design_storage_path: item.designStoragePath || item.design_storage_path || null,
...
function fitProductCardTitles(root = document) {
    ...
    if (scope instanceof Element && scope.matches(productCardTitleFitSelector)) {
        titles.push(scope);
    }
...
function renderProducts(products) {
    ...
    scheduleProductCardTitleFit(productsContainer);
...
function applyStaticCatalogSearchFromUrl() {
    if (typeof window === 'undefined') {
        return;
    }

    const params = new URLSearchParams(window.location.search || '');
    const rawQuery = String(params.get('search') || params.get('q') || '').trim();
    ...
    document.querySelectorAll('[data-catalog-grid]').forEach((grid) => {
        ...
        const existingState = document.querySelector('[data-static-catalog-search-state]');
        if (existingState) {
            existingState.remove();
        }

        const state = document.createElement('div');
        state.setAttribute('data-static-catalog-search-state', '1');
        state.className = 'catalog-search-state';
        state.textContent = visibleCount > 0
            ? `${visibleCount} ${mobileMenuText('produto(s) encontrado(s) para')} "${rawQuery}"`
            : `${mobileMenuText('Nenhum produto encontrado para')} "${rawQuery}"`;
        grid.parentElement?.insertBefore(state, grid);
    });
}
...
// ===== MOBILE MENU =====
function getMobileMenuPath(path) {
    if (typeof SiteRoutes !== 'undefined' && typeof SiteRoutes.getLocalizedPath === 'function') {
        return SiteRoutes.getLocalizedPath(path);
    }

    const isSpanish = String(window.location.pathname || '').startsWith('/es');
    if (!isSpanish) {
        return path;
    }
...
function isMobileMenuCurrent(path) {
    const currentPath = String(window.location.pathname || '/').replace(/\/+$/, '') || '/';
    const targetPath = String(getMobileMenuPath(path) || '/').replace(/\/+$/, '') || '/';

    if (targetPath === '/' || targetPath === '/es') {
...
function enhanceMobileMenu() {
    ...
    if (!mobileMenu || mobileMenuEnhanced) {
        return;
    }
    ...
    mobileMenu.setAttribute('aria-label', mobileMenuText('Menu mobile'));
    ...
    const searchForm = mobileMenu.querySelector('[data-mobile-menu-search]');
    if (searchForm) {
...
function toggleMobileMenu(event) {
    ...
    if (mobileMenu) {
        ...
        document.body.classList.toggle('mobile-menu-is-open', willOpen);
        if (mobileMenuBtn) {
        ...
        if (!willOpen) {
            ...
            const input = mobileMenu.querySelector('#mobile-menu-search-input');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    applyStaticCatalogSearchFromUrl();
