...
<body class="bg-slate-50 text-slate-900" data-analytics-event="view_product" data-analytics-product-id="${escapeHtml(product.id)}" data-analytics-category-slug="${escapeHtml(product.categorySlug)}" data-product-id="${escapeHtml(product.id)}" data-product-name="${escapeHtml(product.nome)}" data-product-category-slug="${escapeHtml(product.categorySlug)}">
  ...
  <main>
    ...
    <section class="bg-white">
      <div class="mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1.08fr),minmax(340px,0.92fr)] lg:px-8 lg:py-14">
        ...
        <div class="flex flex-col gap-5 sm:gap-6">
          <div class="space-y-4">
            <div class="flex flex-wrap items-center gap-2">
              <a href="${SiteRoutes.buildCategoryPath(product.categorySlug)}" class="inline-flex rounded-full bg-slate-900 px-3 py-1.5 text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-white shadow-sm transition hover:bg-slate-800">${escapeHtml(product.categoryLabel)}</a>
              <span class="inline-flex rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-slate-500">Produção rápida</span>
            </div>
            <div class="space-y-3">
            ...
            <div class="flex flex-col gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <div class="mt-2 flex flex-wrap items-end gap-3">
                  ...
                  <span class="inline-flex rounded-full bg-emerald-50 px-3 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.2em] text-emerald-700">Produção ágil</span>
                </div>
              </div>
              <div class="flex flex-col gap-3 sm:items-end">
                <div class="max-w-[14.5rem] rounded-2xl bg-slate-900 px-4 py-3 text-sm text-white shadow-[0_14px_30px_rgba(15,23,42,0.18)]">
                  <span class="block text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-white/60">Resumo rápido</span>
                  <span class="mt-1 block text-sm leading-5 text-white/85">Escolhe as opções, envia o ficheiro e avança para a encomenda.</span>
                </div>
                <a href="${product.personalizePath}" data-personalize-link="true" ${product.categorySlug === 'fly-banner' ? 'data-flybanner-personalize-trigger="true"' : ''} data-product-id="${escapeHtml(product.id)}" data-product-name="${escapeHtml(product.nome)}" data-product-category="${escapeHtml(product.categorySlug)}" data-analytics-event="start_personalization" data-analytics-product-id="${escapeHtml(product.id)}" data-analytics-category-slug="${escapeHtml(product.categorySlug)}" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white shadow-[0_14px_30px_rgba(15,23,42,0.18)] transition hover:-translate-y-0.5 hover:bg-slate-800 sm:hidden">
                  <span>Personalizar</span>
                  <span aria-hidden="true">&rarr;</span>
                </a>
              </div>
            </div>
            <a href="${product.personalizePath}" data-personalize-link="true" ${product.categorySlug === 'fly-banner' ? 'data-flybanner-personalize-trigger="true"' : ''} data-product-id="${escapeHtml(product.id)}" data-product-name="${escapeHtml(product.nome)}" data-product-category="${escapeHtml(product.categorySlug)}" data-analytics-event="start_personalization" data-analytics-product-id="${escapeHtml(product.id)}" data-analytics-category-slug="${escapeHtml(product.categorySlug)}" class="mt-5 hidden w-full items-center justify-center gap-2 rounded-2xl bg-slate-900 px-5 py-3.5 text-sm font-semibold text-white shadow-[0_14px_30px_rgba(15,23,42,0.18)] transition hover:-translate-y-0.5 hover:bg-slate-800 sm:inline-flex">
              <span>Personalizar produto</span>
              <span aria-hidden="true">&rarr;</span>
            </a>
            <p class="mt-4 text-sm leading-6 text-slate-500 sm:hidden">Escolhe as opções e continua para finalizar a encomenda.</p>
            <p class="mt-4 hidden text-sm leading-6 text-slate-500 sm:block">Escolha as opções, envie o design e finalize a encomenda no passo seguinte.</p>
            <div class="mt-4 grid gap-2 sm:hidden">
              <a href="${SiteRoutes.STATIC_PATHS.shipping}" class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 shadow-sm transition hover:border-slate-300 hover:text-slate-900">
                <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-slate-900 text-white">&rarr;</span>
                <span class="min-w-0">
                  <strong class="block text-sm font-semibold text-slate-900">Envios</strong>
                  <span class="block text-xs leading-5 text-slate-500">Prazos, expedição e entrega.</span>
                </span>
              </a>
              <a href="${SiteRoutes.STATIC_PATHS.faq}" class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 shadow-sm transition hover:border-slate-300 hover:text-slate-900">
                <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-slate-900 text-white">?</span>
                <span class="min-w-0">
                  <strong class="block text-sm font-semibold text-slate-900">FAQ</strong>
                  <span class="block text-xs leading-5 text-slate-500">Dúvidas sobre produção e compra.</span>
                </span>
              </a>
              <a href="${SiteRoutes.buildContactPath({ assunto: product.nome })}" class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 shadow-sm transition hover:border-slate-300 hover:text-slate-900">
                <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-slate-900 text-white">+</span>
                <span class="min-w-0">
                  <strong class="block text-sm font-semibold text-slate-900">Pedir apoio</strong>
                  <span class="block text-xs leading-5 text-slate-500">Falar com a equipa comercial.</span>
                </span>
              </a>
            </div>
            <div class="mt-4 hidden gap-3 sm:grid sm:grid-cols-3">
              <a href="${SiteRoutes.STATIC_PATHS.shipping}" class="group rounded-[1.5rem] border border-slate-200 bg-slate-50/80 p-4 text-sm text-slate-600 transition hover:-translate-y-0.5 hover:border-slate-300 hover:bg-white hover:text-slate-900">
                <span class="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-slate-900 shadow-sm transition group-hover:bg-slate-900 group-hover:text-white">&rarr;</span>
                <strong class="block text-sm font-semibold text-slate-900">Envios</strong>
                <span class="mt-1 block leading-6">Prazos, expedição e entrega.</span>
              </a>
              <a href="${SiteRoutes.STATIC_PATHS.faq}" class="group rounded-[1.5rem] border border-slate-200 bg-slate-50/80 p-4 text-sm text-slate-600 transition hover:-translate-y-0.5 hover:border-slate-300 hover:bg-white hover:text-slate-900">
                <span class="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-slate-900 shadow-sm transition group-hover:bg-slate-900 group-hover:text-white">?</span>
                <strong class="block text-sm font-semibold text-slate-900">FAQ</strong>
                <span class="mt-1 block leading-6">Dúvidas sobre produção e compra.</span>
              </a>
              <a href="${SiteRoutes.buildContactPath({ assunto: product.nome })}" class="group rounded-[1.5rem] border border-slate-200 bg-slate-50/80 p-4 text-sm text-slate-600 transition hover:-translate-y-0.5 hover:border-slate-300 hover:bg-white hover:text-slate-900">
                <span class="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-slate-900 shadow-sm transition group-hover:bg-slate-900 group-hover:text-white">+</span>
                <strong class="block text-sm font-semibold text-slate-900">Pedir apoio</strong>
